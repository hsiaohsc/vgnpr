...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_paths", type=int, default=2048)
    ...
    f.write(f"num_paths: {str(args.num_paths)}\n")
    f.write(f"max_width: {str(args.max_width)}\n")
    f.write(f"use_lpips_loss: {str(args.use_lpips_loss)}\n")
    f.write(f"num_iter: {str(args.num_iter)}\n")
    f.write(f"use_blob: {str(args.use_blob)}\n")
    # f.write(f"use_color: {str(args.use_color)}\n")
    f.write('\n')
